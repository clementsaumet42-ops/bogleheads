...
def _creer_onglet_glide_path(wb: openpyxl.Workbook, profil_ref: dict):
    ...
    params_marche = charger_params_projection()
    sim_params = params_marche.get("simulation", {})
    nb_tirages = min(sim_params.get("nb_tirages", 10000), 2000)  # limiter pour vitesse
    seed = sim_params.get("seed", 42)
    ...
    # Colonnes : Actions % (col 3), Obligations % (col 4), Monétaire % (col 5), Immobilier % (col 6)
    ...
    # Construire l'allocation fixe (identique à celle utilisée dans Projection_MonteCarlo)
    alloc_profil = profil_ref.get("allocation_cible_bogleheads", {})
    actions_total = float(alloc_profil.get("actions", 0.65))
    ...
    from src.projection import simuler_monte_carlo, ParametresProjection
    ...
    set_col_width(ws, 1, 12)   # Année
    set_col_width(ws, 2, 8)    # Âge
    set_col_width(ws, 3, 12)   # Actions %
    set_col_width(ws, 4, 14)   # Obligations %
    set_col_width(ws, 5, 13)   # Monétaire %
    set_col_width(ws, 6, 13)   # Immobilier %
    set_col_width(ws, 7, 15)   # Actions Monde %
    set_col_width(ws, 8, 13)   # Actions USA %
    set_col_width(ws, 9, 15)   # Actions Europe %
    set_col_width(ws, 10, 17)  # Actions Émergents %
    set_col_width(ws, 11, 16)  # Défensif total %
    set_col_width(ws, 12, 16)  # Offensif total %
...
def _creer_onglet_rebalancement_flux(wb: openpyxl.Workbook, profil_ref: dict):
    ...
    # Données du profil de référence pour pré-remplissage
    alloc_profil = profil_ref.get("allocation_cible_bogleheads", {})
    ...
    # Pré-remplissage : allocation cible (approximative) comme montant actuel
    classes_info = [
    ...
    data_start_row = row
    montant_cells = []   # (row, col) of editable montant cells
    cible_cells = []     # (row, col) of editable cible cells

    total_row_s2 = data_start_row + len(classes_info)
    # Col letters
    col_montant = "B"
    col_poids_act = "C"
    col_cible = "D"
    ...
    for i, (label, _key, montant_def, cible_def) in enumerate(classes_info):
        ...
        # Col B — montant saisissable (jaune)
        ...
        montant_cells.append((r, 2))

        # Col C — poids actuel % (formule)
        total_ref = f"SUM({col_montant}{data_start_row}:{col_montant}{total_row_s2 - 1})"
        ...
        # Col D — poids cible % (saisissable, jaune)
        ...
        cible_cells.append((r, 4))

        # Col E — écart % = cible - actuel (formule)
    ...
    # Écart > 5 pp → rouge (sous-pondéré significatif)
    ws.conditional_formatting.add(
        ecart_range,
        CellIsRule(operator="greaterThan", formula=["0.05"],
                   fill=_fill(ROUGE_NOK))
    )
    # Écart < -5 pp → rouge (surpondéré significatif)
    ws.conditional_formatting.add(
        ecart_range,
        CellIsRule(operator="lessThan", formula=["-0.05"],
                   fill=_fill(ROUGE_NOK))
    )
    # |écart| ≤ 5 pp → vert (dans les bandes)
    ws.conditional_formatting.add(
        ecart_range,
    ...
    # Construire le portefeuille depuis les valeurs pré-remplies
    kwarg_map = {label: key for label, key, _, _ in classes_info}
    ...
    for env in enveloppes_fiscales:
        ...
        economie_str = f"+{cmp.economie_fiscale:,.0f} €" if cmp.economie_fiscale > 0 else "0 €"
    ...
    # Trouver le premier mois sans classe hors bandes
    ...
    # ── Largeurs de colonnes ──────────────────────────────────────────
    set_col_width(ws, 1, 22)   # Classe
    set_col_width(ws, 2, 22)   # Montant
    set_col_width(ws, 3, 16)   # Poids actuel
    set_col_width(ws, 4, 16)   # Poids cible
    set_col_width(ws, 5, 14)   # Écart / Versement
    set_col_width(ws, 6, 16)   # Écart €
    set_col_width(ws, 7, 16)
    set_col_width(ws, 8, 16)
