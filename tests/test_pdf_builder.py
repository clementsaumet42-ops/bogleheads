...
class TestGenerationPDFProfil1:
    ...
    def test_nb_pages_egal_13(self, pdf_profil1):
        ...
        assert pdf_profil1.nb_pages == 17
...
class TestSixProfils:
    def test_tous_profils_generent_sans_erreur(self, profils, tmp_path):
        ...
        for profil in profils.profils:
            ...
            assert res.nb_pages == 17, f"Profil {profil.id} : {res.nb_pages} pages au lieu de 17"
    ...
    def test_tous_profils_13_pages_pypdf(self, profils, tmp_path):
        ...
        for profil in profils.profils:
            ...
            assert len(reader.pages) == 17, f"Profil {profil.id} : {len(reader.pages)} pages"
...
class TestReproductibilite:
    def test_meme_profil_meme_nb_pages(self, profil1, config_pdf, tmp_path):
        ...
        assert res1.nb_pages == res2.nb_pages == 17
...
class TestExemplesCommites:
    ...
    def test_exemples_13_pages(self):
        ...
        for pdf_path in examples_dir.glob("*_exemple.pdf"):
            ...
            assert len(reader.pages) == 17, (
                f"{pdf_path.name} : {len(reader.pages)} pages au lieu de 17"
            )
...
# ─── Tests S17 — Annexe hypothèses ───────────────────────────────────────────


class TestAnnexeHypotheses:
    """Tests Sprint S17 — page Annexe Hypothèses & Sources."""

    def test_pdf_contient_annexe_hypotheses(self, profil1, config_pdf, tmp_path):
        """Le PDF généré contient la page 'Annexe — Hypothèses retenues et sources'."""
        from pypdf import PdfReader

        out = tmp_path / "annexe_hyp.pdf"
        generer_pdf(profil1, config_pdf, out)
        reader = PdfReader(str(out))
        text = "".join(p.extract_text() or "" for p in reader.pages)
        assert "Hypothèses" in text or "hypothèses" in text

    def test_pdf_annexe_liste_au_moins_5_hypotheses(self, profil1, config_pdf, tmp_path):
        """L'annexe hypothèses liste au moins 5 clés d'hypothèses."""
        from pypdf import PdfReader

        out = tmp_path / "annexe_hyp5.pdf"
        generer_pdf(profil1, config_pdf, out)
        reader = PdfReader(str(out))
        text = "".join(p.extract_text() or "" for p in reader.pages)
        # Les clés prioritaires doivent apparaître
        cles_prioritaires = [
        ...
        cles_trouvees = sum(1 for cle in cles_prioritaires if cle in text)
        assert cles_trouvees >= 5, (
            f"Seulement {cles_trouvees}/5 clés prioritaires trouvées dans l'annexe"
        )

    def test_pdf_17_pages(self, profil1, config_pdf, tmp_path):
        """Le PDF est bien à 17 pages avec l'annexe hypothèses (S17)."""

        out = tmp_path / "s17_pages.pdf"
        result = generer_pdf(profil1, config_pdf, out)
