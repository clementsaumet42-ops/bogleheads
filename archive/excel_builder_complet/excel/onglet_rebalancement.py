...
def creer_onglet_rebalancement(wb: openpyxl.Workbook):
    ...
    for i, (classe, cible, env) in enumerate(zip(classes, cibles, env_pref)):
        ...
        if not is_total:
            ...
            ws.cell(
                row=row, column=4, value=f"={actuelle_col}{row}-{cible_col}{row}"
            ).number_format = "0.0%"
            ...
            ws.cell(
                row=row,
                column=5,
                value=f"=IF({cible_col}{row}>0,ABS({derive_col}{row})/{cible_col}{row},0)",
            ).number_format = "0.0%"
            ...
            ws.cell(
                row=row, column=7, value=f"=ABS({derive_col}{row})*{patrimoine_exemple}"
            ).number_format = "#,##0 €"
    ...
    ws.conditional_formatting.add(
        action_range, CellIsRule(operator="equal", formula=['"OK"'], fill=green_fill)
    )
    ws.conditional_formatting.add(
        action_range, CellIsRule(operator="equal", formula=['"ARBITRER"'], fill=red_fill)
    )
    ws.conditional_formatting.add(
        action_range, CellIsRule(operator="equal", formula=['"SURVEILLER"'], fill=orange_fill)
    )
    ...
    methodes = [
        (
            "1ère priorité : Versements",
            "Orienter les nouveaux versements vers les classes sous-pondérées → coût fiscal = 0",
        ),
        (
            "2ème priorité : Arbitrage intra-enveloppe",
            "PEA, PER, PEE → arbitrage SANS fiscalité (enveloppe = bouclier fiscal)",
        ),
        (
            "3ème priorité : Arbitrage CTO",
            "PFU 31,4% sur les plus-values → calculer le seuil de rentabilité",
        ),
        (
            "Méthode Swedroe (recommandée)",
            "Rebalancer si dérive > 25% de la cible (relatif) OU > 5 pts absolus",
        ),
        ("Fréquence recommandée", "Vérification annuelle + après chaque mouvement de marché > 20%"),
        (
            "Coût fiscal arbitrage CTO",
            "PV × 31,4% → seuil : dérive > 3-5 ans de surperformance de l'allocation cible",
        ),
    ]
    for methode, explication in methodes:
        ...
        ws.cell(row=row, column=2, value=explication).alignment = _align(
